...
test("uses the generic broker route and resolves each inference independently", async () => {
  configureSession();
  const requests = [];
...
test("does not share concurrent token results across prompt authors", async () => {
  configureSession();
  const resolvers = [];
  let requestCount = 0;
  globalThis.fetch = () => {
  ...
  resolvers[0](Response.json({ accessToken: "first", expiresIn: 3600 }));
  resolvers[1](Response.json({ accessToken: "second", expiresIn: 3600 }));
